...
```javascript
{
  ...
  originalUrl: String,      // Required. Full original URL (trimmed). No format validation at schema level — validated at service layer.
  shortCode: String,        // Required. Unique short identifier (min 5 chars). Auto-generated by nanoid if not provided. Unique DB index.
  clickCount: Number,       // Default 0. Denormalized visit count — incremented on each redirect. Avoids COUNT queries on dashboard loads.
  createdAt: Date,          // Auto-set on insert by Mongoose timestamps option. Never modified.
  updatedAt: Date           // Auto-updated by Mongoose on every save/findOneAndUpdate.
}
```

**Indexes:**
- `{ shortCode: 1 }` — unique index for O(1) lookups on every redirect (most frequent query)

**Rationale:**
- `shortCode` generated via `nanoid`; uniqueness enforced at the DB level, not just application level
- `clickCount` denormalized for O(1) dashboard reads at the cost of a minor write on each redirect
- `timestamps: true` handles `createdAt`/`updatedAt` automatically — no manual date management

#### Visit Collection (`backend/models/Visit.js`)
```javascript
{
  _id: ObjectId,            // Auto-generated by MongoDB
  urlId: ObjectId,          // Required. Ref to URL._id. Foreign key linking visit to its parent URL.
  ipAddress: String,        // Optional. Visitor IPv4/IPv6 from req.ip. May be absent in proxy/private network environments.
  userAgent: String,        // Optional. Full User-Agent header string. Used for browser/device analytics.
  timestamp: Date           // Default Date.now (UTC). Set on insert, never updated. Used for time-range queries.
}
```

**Indexes:**
- `{ urlId: 1, timestamp: -1 }` — compound index for fast per-URL visit queries sorted newest-first (statistics & aggregation)

**Rationale:**
- `ipAddress` and `userAgent` are optional — proxies or certain environments may not expose reliable values
- Compound index on `urlId + timestamp` covers both the visit list query and daily aggregation in one index
- Separate collection (instead of embedding visits in URL) allows unbounded visit growth without document size limits
...
**URL Collection**
- `originalUrl`: Required, trimmed string — URL format validated at service layer, not schema
- `shortCode`: Required, unique, min 5 chars — auto-generated by `nanoid`; unique index at DB level
- `clickCount`: Default 0, min 0 — incremented atomically on each redirect
- `createdAt` / `updatedAt`: Auto-managed by Mongoose `timestamps: true`

**Visit Collection**
- `urlId`: Required, ObjectId ref to `URL._id` — a visit without a parent URL is invalid
- `ipAddress`: Optional string — not enforced as required due to proxy/private network environments
- `userAgent`: Optional string — present when browser sends the header
- `timestamp`: Default `Date.now` (UTC), set on insert only — never updated
...
|----------|-----------|-------------|
| **nanoid** | Lightweight, human-readable IDs | UUID, base62 encoding |
...
**Changelog:** v1.1 — Expanded section 2.2 and 5.1 with full field-level documentation matching implemented models (URL.js, Visit.js)
